...
const BirdShitApp = () => {
  ...
  const iconBtn = (onClick: () => void, children: React.ReactNode, title: string) => (
    <button
      onClick={onClick}
      title={title}
      className="p-2 rounded-lg border border-gray-700 bg-gray-800/60 hover:bg-gray-700 text-gray-400 hover:text-white transition-colors"
    >
      {children}
    </button>
  );
    ...
    <div ref={containerRef} className="max-w-4xl mx-auto space-y-4">
      ...
      {mobilePlay && isPortrait && !portraitDismissed && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-4" style={{ background: 'rgba(10,10,14,0.92)' }}>
          ...
          <button onClick={() => setPortraitDismissed(true)} className="mt-2 px-5 py-2 text-sm text-gray-300 border border-gray-600 rounded-lg active:bg-gray-800">
            Continue anyway
          </button>
      ...
      >
        ...
        {mobilePlay && (
          <div className="absolute top-2 right-2 z-10 flex gap-2">
            <button onClick={() => { const m = !soundMuted; setSoundMuted(m); birdShitSound.setMuted(m); }}
              className="p-2 rounded-lg active:scale-90" style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.15)', touchAction: 'none' }}>
              {soundMuted ? <VolumeX className="w-5 h-5 text-white/70" /> : <Volume2 className="w-5 h-5 text-white/70" />}
            </button>
            <button onClick={toggleFullscreen}
              className="p-2 rounded-lg active:scale-90" style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.15)', touchAction: 'none' }}>
              {isFullscreen ? <Minimize className="w-5 h-5 text-white/70" /> : <Maximize className="w-5 h-5 text-white/70" />}
            </button>
          </div>
        )}
        ...
        {/* Mobile play-again */}
        {mobilePlay && gameState !== 'playing' && gameState !== 'upgrading' && gameState !== 'relic_select' && (
          <div className="absolute inset-0 flex items-end justify-center pb-8 pointer-events-none">
            {isOver && <button onClick={startGame} className="btn-primary flex items-center gap-2 px-6 py-3 text-lg pointer-events-auto"><Play className="w-5 h-5" /> Play Again</button>}
          </div>
        )}
        ...
        {!mobilePlay && (
          <div className="flex items-center justify-between w-full mt-3" style={{ maxWidth: CANVAS_W }}>
            ...
            <div className="flex items-center gap-2">
              {iconBtn(
                () => { const m = !soundMuted; setSoundMuted(m); birdShitSound.setMuted(m); },
                soundMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />,
                'Toggle sound'
              )}
              {iconBtn(() => setShowLeaderboard(true), <Trophy className="w-4 h-4" />, 'Leaderboard')}
              {iconBtn(() => setShowSettings(true), <Settings className="w-4 h-4" />, 'Settings')}
            </div>
          </div>
        )}
      </div>
      ...
      {showLeaderboard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setShowLeaderboard(false)}>
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowLeaderboard(false)} className="absolute -top-12 right-0 p-2 text-gray-400 hover:text-white rounded-lg transition-colors">
              <X className="w-6 h-6" />
            </button>
          ...
          <div className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-xl border border-gray-700 bg-gray-900 p-5 space-y-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                />
                {playerName && (
                  <button onClick={() => { setPlayerName(''); localStorage.removeItem('birdshit-player-name'); }} className="p-2 text-gray-400 hover:text-white"><X className="w-4 h-4" /></button>
                )}
              </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <button onClick={() => { const m = !soundMuted; setSoundMuted(m); birdShitSound.setMuted(m); }} className="p-1 rounded hover:bg-gray-800 transition-colors">
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        active ? 'border-802 bg-802/10'
                          : locked ? 'border-gray-800 bg-gray-900/40 opacity-50 cursor-not-allowed'
                    ...
                    >
                      ...
                      {locked && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-lg bg-black/40">
                          <Lock className="w-4 h-4 text-gray-400" /><span className="text-gray-400 text-xs mt-0.5">{skin.unlockScore}</span>
                        </div>
                      )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3 text-xs text-gray-400">
                ...
                <span><span className="text-red-400 font-bold">3 HP</span> start</span>
              </div>
            </div>
          </div>
        </div>
